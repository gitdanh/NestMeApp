...
function Chat(props) {
        ...
        setConversations((prevConversations) => {
          return prevConversations.map((con) => {
            if (con._id == data.conversationId) {
              let lastmsg;
              if (data?.media.length > 0) lastmsg = "Image";
              else lastmsg = data.content;
              return {
                ...con,
                msg_id: data._id,
                lastMsg: lastmsg,
  ...
  useEffect(() => {
    if (socket) {
      // console.log("toi socket");
      if (socket.current) {
        socket.current.on("delete-recieve", (data) =>
          handleRecieveDeleteChat(data)
        );
      }
    }
  }, [socket?.current]);
    ...
    if (!item?.is_deleted.deleted)
      return (
        <ListItem
          ...
          onPress={() => {
            navigation.navigate("SingleChat", { data: item });
            //console.log(item);
            setConversations((prevConversations) => {
              return prevConversations.map((con) => {
                if (con._id == item._id) {
        ...
        >
          <View>
            <Avatar
              source={item?.img === "" ? defaultAvatar : { uri: item.img }}
              rounded
              title={item.name}
              size="medium"
            />
