...
import {
    Dimensions, SafeAreaView, StatusBar, FlatList, Image, ScrollView, StyleSheet, Text, TouchableOpacity, View
} from "react-native";
...
export default function VideoScreen(props) {
        ...
        <View>
            <View style={{ justifyContent: 'space-evenly', width: '100%', alignItems: 'center', flexDirection: 'row', backgroundColor: 'black' }}>
                <TouchableOpacity disabled={!isCameraReady} onPress={() => setIsFlash(!isFlash)} >
                    <Feather style={utils.margin15} name={"zap"} size={25} color="white" />
                </TouchableOpacity>
                <TouchableOpacity disabled={!isCameraReady} onPress={switchCamera}>
                    <Feather style={utils.margin15} name="rotate-cw" size={25} color="white" />
                </TouchableOpacity>
                {type == 0 ?
                ...
                <TouchableOpacity disabled={!isCameraReady} onPress={() => type == 1 ? setType(0) : setType(1)} >
                    <Feather style={utils.margin15} name={type == 0 ? "camera" : "video"} size={25} color="white" />
                </TouchableOpacity>
                <TouchableOpacity onPress={() => setShowGallery(true)} >
                    <Feather style={utils.margin15} name={"image"} size={25} color="white" />
                </TouchableOpacity>
            </View>
    ...
    if (showGallery) {
        return (
            ...
            <ScrollView
                ...
                <View style={{ paddingLeft: 20, paddingRight: 20, marginBottom: 20, display: "flex", flexDirection:"row", alignItems:"center", justifyContent: "space-between"}}>
                    <View style={{display: "flex", flexDirection:"row", alignItems:"center"}}>
                        <AntDesign color={"white"} size={27} name="close" onPress={() => props.navigation.navigate("Home")}/>
                        <Text style={{fontSize: 16, fontWeight: '700', color: 'white', marginLeft: 15}}> New Post</Text>
                    </View>
                    <Text style={{fontSize: 16, fontWeight: '500', color: "#0095f6"}} onPress={() => handleGoToSaveOnGalleryPick()}>Next</Text>
                ...
                <View style={{ justifyContent: 'flex-end', alignItems: 'center', marginRight: 20, marginVertical: 10, flexDirection: 'row' }}>
                    <TouchableOpacity style={{ alignItems: 'center', backgroundColor: 'rgba(255, 255, 255, .2);', borderRadius: 50, borderWidth: 1, borderColor: 'black' }} onPress={() => setShowGallery(false)} >
    ...
    return (
        <View style={{ flex: 1, flexDirection: 'column', backgroundColor: 'black',  paddingTop: StatusBar.currentHeight || 100,}}>
            <View style={{ marginLeft: 20, marginBottom: 20}}><AntDesign color={"white"} size={30} name="close" onPress={() => setShowGallery(true)}/></View>
            <View
                style={[{ aspectRatio: 1 / 1, height: WINDOW_WIDTH }]}>
                {isFocused ?
                    <Camera
